...
const YouTube = () => {
  ...
  useEffect(() => {
    ...
    const movieCategory = Array.from(
      new Set(
        isLoginUser?.Viewing_Record?.map((record) => record.movie_category)
      )
    );
  ...
  useEffect(() => {
    ...
    const categoryFilter = randomCategory
      ? allMovies.filter((movie) =>
          movie.movie_category.includes(randomCategory)
        )
      : [];
    const titleFilter = randomSearchTerm
      ? allMovies.filter((movie) =>
          movie.movie_title.includes(randomSearchTerm)
        )
      : [];
